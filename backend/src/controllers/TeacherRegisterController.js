...
//Función
teacherController.registerTeacher = async (request, response) => {
  //Variables
  const {
    name,
    lastName,
  ...
  try {
    ...
    const existsEmail = await TeacherModel.findOne({ email });
...
teacherController.verifyCode = async (request, response) => {
